...
public static unsafe class TabAddressBook
{
    ...
    public static readonly Dictionary<SortMode, string> SortModeNames = new()
    {
        ...
        [SortMode.World] = "服务 (A-Z)",
        [SortMode.WorldReversed] = "服务 (Z-A)",
        [SortMode.Plot] = "区 (1-9)",
    ...
    public static void DrawBook(AddressBookFolder book, bool readOnly = false)
    {
        ...
        if (ImGui.BeginTable($"##addressbook", 4, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.SizingFixedFit))
        {
            ...
            if (book.SortMode.EqualsAny(SortMode.Name, SortMode.NameReversed))
                entryArray = [.. book.Entries.OrderBy(x => x.Name.NullWhenEmpty() ?? x.GetAutoName()).ThenBy(x => x.Ward).ThenBy(x => x.Plot)];
            else if (book.SortMode.EqualsAny(SortMode.World, SortMode.WorldReversed))
                entryArray = [.. book.Entries.OrderBy(x => ExcelWorldHelper.GetName(x.World)).ThenBy(x => x.Name.NullWhenEmpty() ?? x.GetAutoName())];
            else if (book.SortMode.EqualsAny(SortMode.Ward, SortMode.WardReversed))
                entryArray = [.. book.Entries.OrderBy(x => x.Ward).ThenBy(x => x.Plot).ThenBy(x => x.Name.NullWhenEmpty() ?? x.GetAutoName())];
            else if (book.SortMode.EqualsAny(SortMode.Plot, SortMode.PlotReversed))
                entryArray = [.. book.Entries.OrderBy(x => x.Plot).ThenBy(x => x.Ward).ThenBy(x => x.Name.NullWhenEmpty() ?? x.GetAutoName())];
            else
                entryArray = [.. book.Entries];
            ...
            for (var i = 0; i < entryArray.Count; i++)
            {
                ...
                if (ImGui.BeginPopup($"ABMenu {entry.GUID}"))
                {
                    ...
                    if (entry.Alias != "")
                    {
                        ImGui.MenuItem($"启用别名: {entry.Alias}", ref entry.AliasEnabled);
                    }
                ...
                {
                    if (!S.Data.DataStore.DCWorlds.Contains(ExcelWorldHelper.GetName(entry.World)))
                        wcol = ImGuiColors.DalamudGrey3;
                }
                if (Player.Available && Player.Object.CurrentWorld.RowId == entry.World)
                    wcol = new Vector4(0.9f, 0.9f, 0.9f, 1f);
