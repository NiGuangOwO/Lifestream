...
public class Overlay : Window
{
    ...
    public override void Draw()
    {
        ...
        {
            if (C.ShowAethernet)
                actions.Add(DrawCustomAethernet);
        }
        else if (P.ActiveAetheryte != null)
        {
            if (C.ShowAethernet)
                actions.Add(DrawNormalAethernet);
            if (P.ActiveAetheryte.Value.IsWorldChangeAetheryte() && C.ShowWorldVisit)
                actions.Add(DrawWorldVisit);
            if (C.ShowWards && Utils.HousingAethernet.Contains(P.Territory) && P.ActiveAetheryte.Value.IsResidentialAetheryte())
                actions.Add(DrawHousingWards);
        }
    ...
    private void DrawResidentialAethernet(bool? subdivision = null)
    {
        ...
        void Draw(bool favorites)
        {
            ...
            {
                if (C.Favorites.Contains(x.ID) != favorites)
                    continue;
                if (subdivision != null && x.IsSubdivision != subdivision)
                    continue;
                if (!C.Hidden.Contains(x.ID))
                {
                    var name = (C.Favorites.Contains(x.ID) ? Lang.Symbols.HighQuality + " " : "") + (C.Renames.TryGetValue(x.ID, out var value) ? value : x.Name);
    ...
    private void Popup(IAetheryte x)
    {
        ...
        if (ImGui.BeginPopup($"LifestreamPopup{x.ID}"))
        {
            ...
            if (ImGuiEx.CollectionCheckbox("隐藏", x.ID, C.Hidden))
                EzConfig.Save();
    ...
    public override bool DrawConditions()
    {
        ...
        var ret = canUse != AetheryteUseState.无;
        if (canUse == AetheryteUseState.普通)
        {
