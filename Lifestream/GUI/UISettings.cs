...
internal static unsafe class UISettings
{
    ...
    private static void DrawGeneral()
    {
        ...
        .Widget(() =>
        {
            ...
            ImGui.Checkbox($"访问 服务器/大区 后传送到指定的以太网目的地", ref C.WorldVisitTPToAethernet);
            if (C.WorldVisitTPToAethernet)
            {
                ...
                ImGui.Checkbox($"只能通过指令传送，不能通过悬浮窗传送。", ref C.WorldVisitTPOnlyCmd);
                ImGui.Unindent();
            }
            ...
            ImGui.Checkbox("启用传送完成后的托盘通知", ref C.EnableNotifications);
        ...
        .Widget(() =>
        {
            ...
            if (Player.Available)
            {
                ImGuiEx.Text($"为 {Censor.Character(Player.NameWithWorld)}:");
                ImGui.Indent();
                if (ImGui.RadioButton("使用全局设置", !custom))
                {
                    C.PropertyPrioOverrides.Remove(Player.CID);
                }
                ImGui.SameLine();

                if (ImGui.RadioButton("使用个性化设置", custom))
                {
                    if (!custom)
                    {
    ...
    private static void DrawOverlay()
    {
        ...
        .Widget(() =>
        {
            ...
            if (C.Enable)
            {
                ...
                ImGui.Checkbox("固定 Lifestream 悬浮窗位置", ref C.FixedPosition);
                if (C.FixedPosition)
                {
                    ImGui.Indent();
                    ImGui.SetNextItemWidth(200f.Scale());
