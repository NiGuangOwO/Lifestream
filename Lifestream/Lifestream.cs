...
public unsafe class Lifestream : IDalamudPlugin
{
    ...
    public Lifestream(DalamudPluginInterface pluginInterface)
    {
        ...
        new TickScheduler(delegate
        {
            ...
            EzCmd.Add("/li", ProcessCommand, "自动将跨服到指定的服务器（用第一个字匹配），如果没有指定，则返回到原始服务器，如果靠近以太网，则传送到以太网目的地。以太网目的地也可以在目标服务器旁边指定。");
            DataStore = new();
    ...
    internal void ProcessCommand(string command, string arguments)
    {
        if (arguments == "stop")
        {
            Notify.Info($"放弃 {TaskManager.NumQueuedTasks + (TaskManager.IsBusy?1:0)} 了个任务");
            TaskManager.Abort();
            followPath?.Stop();
        }
